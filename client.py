...
def client_cek_servis_via_api(plat_nomor, thread_name):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[{thread_name}] Error untuk plat {plat_nomor}: Status {response.status_code}")
    except requests.exceptions.Timeout:
        print(f"[{thread_name}] permintaan timeout untuk plat nomor: {plat_nomor}")
    except requests.exceptions.RequestException as e:
        print(f"[{thread_name}] pesan error untuk plat nomor: {plat_nomor}. Error: {str(e)}")
    print(f"[{thread_name}] selesai memproses {plat_nomor} tidak valid.")
